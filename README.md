...
It combines a **Duolingo-inspired playful desktop GUI** (`#FF9D9D` coral theme with chunky 3D-pressed buttons and subtle retro pixel-art badges) for casual and visual workflows with a **headless CLI & Ratatui TUI** for power users, remote SSH administration, and scripts.
...
- 📸 **Safety Checkpoint & Snapshot Verification**: Critical system purges and tweaks verify or trigger a pre-cleanup snapshot before touching system files (Snapper / Timeshift on Btrfs Linux, Volume Shadow Copy / System Restore on Windows).
- 🔍 **Non-Destructive Dry-Run Scanner**: Recursively inspects candidate paths with `lstat` to prevent following symlinks out of bounds. Checks active process file locks (`/proc` on Linux and Restart Manager on Windows) before flagging any item.
...
- **Branded splash + first-run onboarding**: a brief animated splash on launch, followed (once, on first run) by a skippable 4-step orientation explaining what a dry-run scan is, what the automatic safety snapshot does, and where rollback lives.
